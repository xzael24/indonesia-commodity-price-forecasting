...
def load_processed_data(commodity: str = "Beras"):
    ...
    print(f"[INFO] Memuat data yang sudah diproses untuk {commodity}...")
    ...
    print(f"  [OK] Input shape: {X_train.shape} | Output shape: {y_train.shape}")
    return X_train, X_val, X_test, y_train, y_val, y_test, scaler_data


def train_single_commodity(com: str) -> dict:
    """Melatih dan mengevaluasi ke-4 model untuk satu komoditas tertentu."""
    print(f"\n{'='*50}")
    print(f"  MELATIH KOMODITAS: {com.upper()}")
    print(f"{'='*50}")
    ...
    for model_name in ["lstm", "gru", "tft", "nlinear"]:
        print(f"\n  Model: {model_name.upper()} ({com})")

        model   = get_model(model_name, input_size, target_idx=target_idx)
        history = train_model(
        ...
        com_results[model_name] = {
            "MAE" : float(metrics["MAE"]),
            "RMSE": float(metrics["RMSE"]),
            "MAPE": float(metrics["MAPE"]),
        }

    # Update results.json secara atomic / incremental
...
def run_training():
    ...
    for com in COMMODITIES:
        results[com] = train_single_commodity(com)
